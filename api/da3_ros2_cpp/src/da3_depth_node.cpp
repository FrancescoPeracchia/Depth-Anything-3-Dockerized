...
size_t header_cb(char *buffer, size_t size, size_t nitems, void *userdata)
{
  ...
  // FastAPI returns these as response headers
  if (key == "x-width") {
...
class Da3DepthNode : public rclcpp::Node
{
public:
  Da3DepthNode() : Node("da3_depth_node")
  {
    ...
    sub_ = this->create_subscription<sensor_msgs::msg::Image>(
      ...
      std::bind(&Da3DepthNode::image_cb, this, std::placeholders::_1));

    curl_global_init(CURL_GLOBAL_DEFAULT);

    RCLCPP_INFO(get_logger(), "da3_depth_node listening on %s, publishing %s, backend=%s",
      input_topic_.c_str(), output_topic_.c_str(), backend_url_.c_str());
  }

  ~Da3DepthNode() override
  {
    curl_global_cleanup();
  }
...
private:
  ...
  {
    cv::Mat src;
    bool src_is_bgr_from_decode = false;
    {
      ...
      try {
        ...
        // Some publishers produce malformed Image messages (e.g. step==0).
        // Try a minimal fallback by wrapping msg->data directly.
        ...
        if (enc == "rgb8" || enc == "bgr8") {
          ...
          RCLCPP_WARN(get_logger(), "cv_bridge conversion failed: %s (unsupported encoding fallback: %s)", e.what(), enc.c_str());
          return;
        }

        const size_t expected_step = static_cast<size_t>(msg->width) * static_cast<size_t>(channels);
        const size_t expected_size = static_cast<size_t>(msg->height) * expected_step;
        if (msg->data.size() < expected_size) {
          // Some publishers incorrectly publish compressed bytes in a sensor_msgs/Image.
          // Try to decode as JPEG/PNG instead of failing.
          ...
          if (decoded.empty()) {
            RCLCPP_WARN(
              get_logger(),
              "cv_bridge conversion failed: %s (data too small: %zu < %zu, and imdecode failed)",
              e.what(),
              msg->data.size(),
              expected_size);
            return;
          ...
          src = decoded;
          src_is_bgr_from_decode = true;
        } else {
          const size_t step = (msg->step >= expected_step) ? static_cast<size_t>(msg->step) : expected_step;
          src = cv::Mat(
    ...
    cv::Mat bgr;
    if (src.channels() == 3) {
      // Try to normalize to BGR for JPEG encoding.
      if (!src_is_bgr_from_decode && msg->encoding == "rgb8") {
      ...
      RCLCPP_WARN(get_logger(), "Unsupported channel count: %d", src.channels());
      return;
    }
    ...
    std::vector<uint8_t> encoded;
    {
      ...
      if (!cv::imencode(".jpg", bgr, encoded, params)) {
        RCLCPP_WARN(get_logger(), "Failed to JPEG-encode frame");
        return;
      }
    }
    ...
    const auto ok = call_backend(encoded, depth_bytes, meta);
    if (!ok) {
      return;
    }
  ...
  url << join_infer_image_url(backend_url_)
    ...
    curl_mime_data(field, reinterpret_cast<const char *>(jpeg_bytes.data()), static_cast<size_t>(jpeg_bytes.size()));
    ...
    if (http_code != 200) {
      RCLCPP_WARN(get_logger(), "Backend HTTP %ld (url=%s)", http_code, url.str().c_str());
      return false;
    }
...
int main(int argc, char **argv)
{
  ...
  auto node = std::make_shared<Da3DepthNode>();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}
